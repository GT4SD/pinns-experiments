...
class Solver():
    ...
    def solution_prior_321(self, args, wavefunction):
    ...
    def fourier_prior(self, args, f):
        basis = FourierBasis(max_k=10, input_dimension=3, output_dimension=2, axis=1) # axis=1 corresponds to the polar variable theta
        representation = basis.compute(args)
        return representation - f

    def pde(self, args, wavefunction, **quantum_numbers): # TODO: have not been tested yet, implementation in cartesian coordinates
    ...
    def pde_polar(self, args, wavefunction, **quantum_numbers):
        '''
        args = (r, theta, phi)
    ...
    def create_model(self):
        ...
        if self.network_params["strict_boundary"]:
            ...
            data = CustomPDE(geom, self.pde_polar, bcs=[bc_strict_u, bc_strict_v, bc_u, bc_v, bc_g_u, bc_g_v],\
                num_domain=self.network_params["num_domain"], num_boundary=self.network_params["num_boundary"], pde_extra_arguments=quantum_numbers)
        else:
            data = CustomPDE(geom, self.pde_polar, bcs=[bc_u, bc_v, bc_g_u, bc_g_v],\
                num_domain=self.network_params["num_domain"], num_boundary=self.network_params["num_boundary"], pde_extra_arguments=quantum_numbers)
        # ---------------------------------
        ...
        if self.network_params["backbone"] == "FNN":
            ...
            net = dde.maps.FNN(self.network_params["layers"], "tanh", "Glorot normal")
        elif self.network_params["backbone"] in ["ResNet", "HNN"]:
                ...
                if self.network_params["backbone"] == "ResNet":
                    net = dde.maps.ResNet(input_size, output_size, num_neurons, num_blocks, "tanh", "Glorot normal")
                else:
                    from pinnse.custom_networks.hnn import HNN
                    net = HNN(input_size, output_size, num_neurons, num_blocks, "tanh", "Glorot normal")
        else:
            raise NotImplementedError("Only FNN, ResNet, HNN backbones are experiment ready!")
    ...
    def plot(self, rmax, p_gt, p_pred, save=True):
        quantum_numbers = self.experiment_params["quantum_numbers"]
        n,l,m = quantum_numbers.values()
        ...
        if save:
            plt.savefig(self.results_path + '{n}{l}{m}.png'.format(n=n, l=l, m=m))
        else:
            plt.show()
    ...
    def main(self):
        ...
        self.results_path = self.results_path + folder
        import os
        ...
        if self.prior_params is not None:
            prior_name = self.prior_params["name"]
            prior_num_domain, prior_num_boundary = self.prior_params["num_domain"], self.prior_params["num_boundary"]
            prior_optimizer, prior_lr, prior_epochs = self.prior_params["training_params"].values()
            assert prior_name in ["fourier_prior"], "Prior not defined!"
            if prior_name == "fourier_prior":
                prior_data = CustomPDE(geom, self.fourier_prior, bcs=[], \
                    num_domain=prior_num_domain, num_boundary=prior_num_boundary)
            prior_save_path = self.results_path + prior_name + "/model"
            compile_train_args = {'optimizer':prior_optimizer, 'lr':float(prior_lr), 'epochs':prior_epochs}
        ...
        for i in range(len(self.network_params["lrs"])):
            ...
            model.compile(optimizer=optimizer, lr=lr, loss_weights=self.network_params["loss_weights"], loss="MSE")
