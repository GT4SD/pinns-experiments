...
class Solver():
    ...
    def initial_prior_210(self, args, wavefunction):
        n = 2
        Z = 1
        E_n = - (electron_mass* Z**2 * e**4) / (8 * epsilon_0**2 * h**2 * n**2)
    ...
    def create_model(self):
        ...
        if self.network_params["strict_boundary"]:
            ...
            data = CustomTimePDE(geomtime, self.pde_polar, ic_bcs=[bc_u, bc_v, bc_g_u, bc_g_v, ic_u, ic_v, bc_strict_u, bc_strict_v], \
        ...
        if self.network_params["backbone"] == "FNN":
            ...
            raise NotImplementedError("Only FNNm and ResNet backbones are experiment ready!")
    ...
    def plot(self, rmax, p_gt, p_pred, timestep, save=True):
        ...
        if save:
            plt.savefig(self.results_path + '{n}{l}{m}_time{timestep}.png'.format(n=n, l=l, m=m, timestep=timestep))
        else:
            plt.show()

    def main(self):
        quantum_numbers = self.experiment_params["quantum_numbers"] # quantum numbers of the initial state
        ...
        for time_point in time_points:
            ...
            self.plot(rmax, p_gt, p_pred, time_point)
