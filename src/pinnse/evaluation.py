...
from tensorflow.python.framework.ops import _SESSION_RUN_LOCK_GROUP
import yaml
...
from pinnse.physical_quantities import Evaluator
...
from pinnse.schrodinger_eq_exact_values import TISE_hydrogen_exact, TDSE_unperturbed_exact,\
     TISE_stark_effect_exact, TDSE_one_level_AC_exact
from pinnse.utils import count_zero_in_decimal_number
...
class EquationSystemTest(object):
    ...
    def solve(self):
        ...
        ic_u = dde.DirichletBC(timedomain, self.ic_func_u, lambda _, on_boundary: on_boundary, component=0)
        ic_v = dde.DirichletBC(timedomain, self.ic_func_v, lambda _, on_boundary: on_boundary, component=1)

        data = dde.data.PDE(timedomain, self.pde, [ic_u, ic_v], num_domain=500, num_boundary=200)
...
class PINNTest(object):
    ...
    def __init__(self, script=None, save_pth=None, config_path=None) -> None:
        super().__init__()
        assert script in ["TISE_hydrogen", "TISE_hydrogen_without_decomposition",\
             "TISE_stark_effect", "TDSE_unperturbed", "TDSE_one_level_AC", "TDSE_laser_field"], "Experiment script non-existent!"
        self.script = script
        self.save_pth = save_pth
        self.config_path = config_path # in case of TDSE_one_level_AC
        with open(config_path, "r") as file:
            self.config = yaml.load(file, Loader=yaml.FullLoader)
        self.experiment_params = self.config["experiment_params"] # params like quantum_numbers, electric field etc.
        self.network_params = self.config["network_params"]
        self.results_path = self.config["results_path"]

    def _model_setup(self):
        sys.path.append("/Users/lat/Desktop/Code/pinns-experiments/schrodinger_equation/")
    ...
    def evaluate_metrics(self, sampled_points=10000):
        ...
        if self.script == "TDSE_unperturbed":
            ...
            print("--------------------------")
            print("Relative Error of Complete Prob. Distribution: ", evaluator_tot.relative_error())
            print("Absolute Error of Complete Prob. Distribution: ", evaluator_tot.absolute_error())
            ...
            predicted_u, predicted_v = model.predict(X_test)[:,0:1], model.predict(X_test)[:,1:2] # real part
            ## NORMALIZATION ##
            normalization_constant = self.solver.normalize_output(model=model)
            predicted = (normalization_constant * predicted_u) ** 2 + (normalization_constant * predicted_v) ** 2
            ####################
            ...
            from TISE_hydrogen import create_model_R_nl, create_model_f_lm, create_model_g_m
            ...
            # R MODEL
            R_model, geom, data, net = create_model_R_nl(self.experiment_params, R_params)
            R_model.compile("adam", lr=1e-3, loss='MSE')
            with open(self.save_pth + "model_R/" + 'best_step.txt') as f:
                    best = f.readlines()[0]
            R_model.restore(self.save_pth+"model_R/"+"model.ckpt-" + best, verbose=1)
            ...
            if f_params["prior"] == "Fourier":
                f_model, geom, data, net, prior_data, prior_save_path, compile_train_args =\
                    create_model_f_lm(self.experiment_params, f_params, self.results_path)
            else: 
                f_model, geom, data, net = create_model_f_lm(self.experiment_params, f_params, self.results_path)
            f_model.compile("adam", lr=1e-3, loss='MSE')
            with open(self.save_pth + "model_f/" + 'best_step.txt') as f:
                    best = f.readlines()[0]
            f_model.restore(self.save_pth+"model_f/"+"model.ckpt-" + best, verbose=1)
            ...
            # g MODEL
            g_model, geom, data, net = create_model_g_m(self.experiment_params, g_params)
            g_model.compile("adam", lr=1e-3, loss='MSE')
            with open(self.save_pth + "model_g/" + 'best_step.txt') as f:
                    best = f.readlines()[0]
            g_model.restore(self.save_pth+"model_g/"+"model.ckpt-" + best, verbose=1)
            ...
            R_nl_gt, f_lm_gt, g_m_gt = TISE_hydrogen_exact(r_metric.reshape(-1), theta_metric.reshape(-1), phi_metric.reshape(-1), n,l,m)
            wavefunction_gt = R_nl_gt*f_lm_gt*g_m_gt
            gt = wavefunction_gt**2 # Probability distribution

            wavefunction_pred = R_nl_pred * f_lm_pred * g_m_pred[:,0:1] # take the real part
            ...
            supervised_input, supervised_output, supervised_output_im = self.solver.data_loader()
            ...
            if "resnet" in self.config_path:
                if len(var.eval(session=model.sess).shape) != 1: 
                    ...
                    if (i==0) or (i==len(tf.compat.v1.trainable_variables())-2):
                        ...
                        to_plot = np.hstack([to_plot, var.eval(session=model.sess).flatten()])
                    # print(var)
                ...
                if len(var.eval(session=model.sess).shape) != 1: 
                    axs[plot_counter].hist(var.eval(session=model.sess).flatten(), bins=100, range=(-0.5, 0.5))
                    plot_counter += 1
            #print(var.eval(session=model.sess).shape)
        plt.show()
                

    def _plot(self, p_gt, p_pred, rmax=20, title=None, results_path=None, fig_name=None):
        ...
        plt1 = ax1.imshow(p_gt, vmin=0, vmax=vmax, \
            extent=[-rmax, rmax, -rmax, rmax], interpolation='none',origin='lower', cmap=cm.hot)
        ...
        plt2 = ax2.imshow(p_pred, vmin=0, vmax=vmax, \
            extent=[-rmax, rmax, -rmax, rmax], interpolation='none',origin='lower', cmap=cm.hot)
    ...
    def plots(self):
        ...
        if self.script == "TDSE_unperturbed":
            ...
            for time_point in time_points:
                ...
                title = '{n},{l},{m} ORBITAL AT TIME t={timestep}'.format(n=n, l=l, m=m, timestep=time_point)
            ...
            self._plot(p_gt, p_pred, rmax, title) # it makes sense to define plot function just once!
            ...
            title = '{n},{m} MANIFOLD, ENERGY: {e}'.format(n=n, m=m, e=np.rint(ener)) + "$\Delta$E"
            self._plot(p_gt, p_pred, rmax, title)
